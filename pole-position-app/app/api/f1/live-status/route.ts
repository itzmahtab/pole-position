  ...
  const result = await fetchWithFallback<LiveStatus>(
    {
      primary: async () => {
        ...
        // Sessions are a separate OpenF1 call
      ...
      fallback: async () => {
        ...
        const sessions: SessionSummary[] = buildSessionsFromRace(race);
        const raceStart = new Date(meeting.date_start);
        const state: LiveStatus["state"] = raceStart > now ? "upcoming" : "finished";
...
function deriveLiveStatus(
  ...
  if (activeSession) {
    ...
    const gapMs = new Date(futureSessions[0].date_start).getTime() - now.getTime();
...
function buildSessionsFromRace(race: Record<string, unknown>): SessionSummary[] {
